...
def hebrew(device: str, setup: str, exteroception: bool) -> Dict[str, Collection[str]]:
    ...
    btnext = "ללחוץ רווח " if device == "keyboard" else "ללחוץ על העכבר"
    texts = {
        "done": "סיימת את המטלה. תודה על השתתפותך!",
        "slower": "אטי",
        "faster": "מהיר",
        "checkOximeter": "צריך לוודא כי האלקטרודות מונחות היטב",
        "stayStill": "בבקשה לא לזוז בזמן ההקלטה",
        "tooLate": "מאוחר מדי",
        ...
        "textNext": f"  {btnext} להמשך ",
        "textWaitTrigger": "Waiting for fMRI trigger...",
        "Decision": {
            "Intero": "האם הצלילים מהירים או אטיים מהדופק?",
            "Extero": "האם הצלילים הנוכחיים מהירים או אטיים מהצלילים הקודמים?",
        },
        "Confidence": "עד כמה את/ה בטוח/ה בהחלטה שלך?",
    }

    if device == "keyboard":
        texts["responseText"] = "אם הם אטיים, ללחוץ על המקש למטה, או למעלה אם הם מהירים"
    elif device == "mouse":
        texts["responseText"] = "אם הם אטיים, להקליק שמאלה, או ימינה אם הם מהירים"
    texts[
        "Tutorial1"
    ] = """בניסוי זה אנחנו נקליט את הלב שלך ונשמיע צלילים מהירים 
    או אטיים מקצב פעימות הלב.
    
    המטרה שלך, היא להחליט האם הצלילים מהירים 
    או איטיים מקצב הלב כפי שחשת אותו"""
    texts[
        "Tutorial2"
    ] = """כשמוצג איור זה, צריך להאזין ללב למשך 5 שניות בעיניים פקוחות. 
    נסו לא לזוז, מכיוון שאנחנו מקליטים את הדופק שלך.

    מותר להאזין רק לתחושה הפנימית של הדופק, ולא למדוד אותו באף דרך אחרת.
...
"""
    ...
    texts[
        "Tutorial3_icon"
    ] = f"""לאחר ההקשבה ללב, יוצג שוב האיור ותושמע סדרה של צלילים.
        
        צריך להחליט, האם סדרת הצלילים מהירה ({moreResp}) או אטית ({lessResp}) מקצב הלב שהורגש.
        
        הצלילים בסדרה השניה תמיד יהיו מהירים או אטיים מקצב הלב. במידה ולא בטוחים, צריך לנחש.
        
        בלחיצה על רווח יתחיל האימון.
        """
    texts[
        "Tutorial3_responses"
    ] = f"""בחצי מהמקרים, במקום שיוצג איור של לב יוצג איור של רמקול. 
    
    במקרים אלו, צריך להאזין לסדרה של צלילים, במקום ללב."""

    if exteroception is True:
        texts[
            "Tutorial3bis"
        ] = f"""בחצי מהמקרים, במקום שיוצג איור של לב, 
    יוצג איור של רמקול.          
    
          במקרים אלו, צריך להאזין לסדרה של צלילים, במקום ללב."""
        texts[
            "Tutorial3ter"
        ] = f"""לאחר ההקשבה לצלילים, יוצג שוב האיור ותושמע סדרה נוספת של צלילים.
        
        צריך להחליט, האם סדרת הצלילים השנייה מהירה ({moreResp}) או אטית ({lessResp}) מהסדרה הקודמת.
        
        הצלילים בסדרה השניה תמיד יהיו מהירים או אטיים מסדרת הצלילים הקודמת. במידה ולא בטוחים, צריך לנחש.
...
בחלק זה, הציון הגבוה ביותר (8) אומר בטחון מוחלט והציון הנמוך ביותר (1) אומר ניחוש מוחלט.

             יש להקיש ימינה או שמאלה כדי למקם את תשובתך על הסקאלה ולסיום להקיש רווח
            """

    texts[
        "Tutorial5"
    ] = """מטלה זו תחזור על עצמה לאורך הניסוי.

כאשר המטלה נהיית מאוד קשה, ההבדל בין קצב הלב האמיתי שלך ובין הצלילים העוקבים 
עלול להיות מאוד קטן.

בשלב זה, כדאי לנסות להשתמש בכל הטווח של הסקאלה שמייצגת בטחון עצמי, 
כדי לשקף את חוסר הוודאות האישית שלך. 

ככל שרמת הקושי עולה לאורך המטלה, תחושה של בטחון מוחלט 
 בתשובות שלך יהפוך להיות נדיר יותר."""

    texts[
        "Tutorial6"
    ] = """ההדרכה הסתיימה. אם ישנן שאלות נוספות, זה הזמן לשאול את הנסיין. 
    במידה ואין שאלות, ניתן להתחיל את המטלה"""
    texts[
        "Tutorial7"
