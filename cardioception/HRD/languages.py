...
def hebrew(device: str, setup: str, exteroception: bool) -> Dict[str, Collection[str]]:
    ...
    texts[
        "Tutorial2"
    ] = """כשמוצג איור זה, צריך להאזין ללב למשך 5 שניות. 
    לנסות לא לזוז בפרק זמן זה, מכיוון שאנחנו מקליטים את הדופק שלך.

    מותר להאזין רק אל התחושה הפנימית של הדופק, ולא למדוד אותו באף דרך אחרת.
...
"""
    ...
    יוצג איור של רמקול.          
        ...
        texts[
            "Tutorial3ter"
        ] = f"""אחרי כל סדרה של צלילים תושמע סדרה נוספת של צלילים.
...
"""
    texts[
        "Tutorial4"
    ] = """לאחר כל ההחלטה, תוצג שאלה על רמת בטחון שלך לגבי התשובה שספקת.
...
בשלב זה, כדאי לנסות להשתמש בכל הספקטרום של הסקאלה שמייצגת בטחון עצמי, 
